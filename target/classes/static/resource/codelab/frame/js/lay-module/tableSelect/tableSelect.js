layui.define(['table', 'jquery', 'form'], function (exports) {
    ...
    tableSelect.prototype.render = function (opt) {
        ...
        opt.checkType = opt.checkType || 1;  // 1单选 2多选

        opt.disabledKey = opt.disabledKey || '';
        opt.disabledValues = opt.disabledValues || [];
        ...
        elem.off('click').on('click', function(e) {
            ...
            tableBox += '</form>';
            tableBox += '<button style="float:right;" class="layui-btn layui-btn-normal layui-btn-sm tableSelect_btn_select">选择<span></span></button>';
            tableBox += '</div>';
...
//                    checkedData = table.checkStatus(tableName).data
                    changedCheckedData = table.checkStatus(tableName).data;
                }
            ...
            //
            function setDisabled(res, curr, count) {
                if(opt.disabledKey == '' || !opt.disabledValues.length){
                     return;
                }
                for(var i = 0; i < res.data.length; i++){
                    if(opt.disabledValues.indexOf(res.data[i][opt.disabledKey]) > -1) {
                        var index= res.data[i]['LAY_TABLE_INDEX'];
                        var checkbox = $('#'+tableName+'').next().find('tr[data-index=' + index + '] input[type="checkbox"]');
                        checkbox.prop('checked', false).next().addClass('layui-form-disabled');
                        var radio  = $('#'+tableName+'').next().find('tr[data-index=' + index + '] input[type="radio"]');
                        console.log(radio);
                        radio.prop('checked', false).next().addClass('layui-form-disabled').find("i").html('');
                    }
                }
            }
            ...
            // Fix：单选时，翻页后会将新选中的和原先已选中过的都选上
            function defaultChecked (res, curr, count){
                if(opt.checkType == 1 && changedCheckedData.length) {
                    checkedData = [].concat(changedCheckedData);
                    return;
                }
                if(opt.checkedKey && elem.attr('ts-selected')){ // (checkType == 2) &&
            ...
            table.on('rowDouble('+tableName+')', function(obj){
                ...
                if (checkStatus.data[0].disabled == true) {
                    return false;
                }
