 ...
 */
layui.define(["element","laytpl" ,"jquery", "DZUtils"], function (exports) {
    ...
    var miniMenu = {
        ...
        compileMenu: function(menu,isSub){
            var menuHtml = '<li {{#if( d.menu){ }}  data-menu="{{d.menu}}" {{#}}} class="layui-nav-item menu-li {{d.childOpenClass}} {{d.className}}"  {{#if( d.id){ }}  id="{{d.id}}" {{#}}}> <a  title="{{d.title}}" {{#if( d.path){ }} layuimini-href="{{d.path}}" {{#}}} {{#if( d.target){ }}  target="{{d.target}}" {{#}}} href="javascript:;">{{#if( d.icon){ }}  <i class="fa {{d.icon}}"></i> {{#}}} <span class="layui-left-nav">{{d.title}}</span></a>  {{# if(d.children){}} {{d.children}} {{#}}} </li>' ;
            if(isSub){
                menuHtml = '<dd class="menu-dd {{d.childOpenClass}} {{ d.className }}"> <a href="javascript:;" title="{{d.title}}" {{#if( d.menu){ }}  data-menu="{{d.menu}}" {{#}}} {{#if( d.id){ }}  id="{{d.id}}" {{#}}} {{#if(( !d.child || !d.child.length ) && d.path){ }} layuimini-href="{{d.path}}" {{#}}} {{#if( d.target){ }}  target="{{d.target}}" {{#}}}> {{#if( d.icon){ }}  <i class="fa {{d.icon}}"></i> {{#}}} <span class="layui-left-nav"> {{d.title}}</span></a> {{# if(d.children){}} {{d.children}} {{#}}}</dd>'
            }
            return laytpl(menuHtml).render(menu);
        },
        compileMenuContainer :function(menu,isSub){
            var wrapperHtml = '<ul class="layui-nav layui-nav-tree layui-left-nav-tree {{d.className}}" lay-filter="nav" id="{{d.id}}">{{d.children}}</ul>' ;
            if(isSub){
        ...
        renderMultiModule: function (menuList, menuChildOpen) {
            ...
            var headerMenuHtml = this.each(menuList, function (index, val) { //顶部菜单渲染
                ...
                if(index >= 3){
                    headerPcMoreMenuHtml +=me.compileMenu({ id:id,menu:menu,id:id,icon:val.icon, title:val.menuName, },true);
                }
            ...
            if(menuList.length >= 3){
                $('.pc-more-wrap').show();
                $('.layuimini-menu-header-pc-more').html(headerPcMoreMenuHtml); //pc端更多下拉
            } else {
                $('.pc-more-wrap').hide();
            }
            element.init();
        ...
        listen: function () {
            ...
            element.on("nav(nav)", function (data) {
                var url = $(this).attr("layuimini-href");
                DZUtils.ajaxPost("/dz-ftsp/frame/menu/oper/log/insert", {url: url}, function (res) {
                    console.log("菜单操作日志添加成功！");
                })
            })
        },
