...
var ZYFILE = {
	...
	funUploadFile : function(file){
		...
		var groupName = $("#status_info").attr("user-attr");
...
//					edit             : true,  						      // 是否可以编辑文件（裁剪）
			...
			this.createHtml = function(){
				...
				if(para.dragDrop){
		            ...
		            html += '				<div id="status_info" class="info" user-attr='+ para.groupName +'>选中0张文件，共0B。</div>';
		            html += '				<div class="btns">';
				    ...
				    html += '					</a>';
						html += '				</div>';
						html += '				<div class="upload_file_text"></div>';
					html += '			</div>';
					html += '		</div>';
					html += '		<div class="upload_submit">';
					html += '			<button type="button" id="fileSubmit" class="upload_submit_btn">确认上传文件</button>';
			...
			this.createCorePlug = function(){
				var params = {
					...
					onSelect: function(selectFiles, allFiles) {
						if(allFiles.length > para.maxFileNum){
							alert('一次只能上传' + para.maxFileNum + '个文件');
							allFiles.pop();
							return
						}
			...
			this.addEvent = function(){
				...
				$(".upload_btn").bind("click", function(e){
					...
					if(ZYFILE.funReturnNeedFiles().length > 0){
						if(ZYFILE.funReturnNeedFiles().length > para.maxFileNum){
							alert('一次只能上传' + para.maxFileNum + '个文件');
						}else{
							$("#fileSubmit").click();
						}
